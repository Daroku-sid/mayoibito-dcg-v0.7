...
/* =====================================================================
   使うデッキを引く（v0.4 Stage F）
   ---------------------------------------------------------------------
   v0.3までは、対戦で使えるのは decks.js に書いた公式デッキだけでした。
   v0.4からは自作デッキでも遊べるようにします。

   ただし game.js の中身は変えたくないので、
   「デッキIDから中身を引く」ところだけを1か所にまとめ、
   公式に無いIDのときだけ、外から預かった自作デッキを返します。
   預かりものが無ければ、これまでどおり DECKS を引くだけです。
   ===================================================================== */
const RuntimeDecks = {

  /* 対戦のあいだだけ持つ自作デッキ。deckId → decks.js と同じ形 */
  custom: {},

  /** 対戦を始める前に預ける */
  set: function (deckId, def) {
    this.custom[deckId] = def;
  },
  ...
  get: function (deckId) {
    if (DECKS[deckId]) return DECKS[deckId];
    return this.custom[deckId] || null;
  },
};

/** デッキIDから中身を引く。公式にも自作にも同じように使えます */
function deckDefOf(deckId) {
  return RuntimeDecks.get(deckId);
}
...
const Game = {
  ...
  start: function (firstSide, seedInput, options) {
    ...
    const labels = {
      village: (opt.labels && opt.labels.village) || deckDefOf(decks.village).label,
      mansion: (opt.labels && opt.labels.mansion) || deckDefOf(decks.mansion).label,
    };
  ...
  wouldResolveMeaningfully: function (side, inst) {
    ...
    if (m.type === 'event') {
      switch (inst.cardId) {
        ...
        // 引き戻す力：トラッシュから〔村〕の人間／怪異／グッズを回収する。
        // 回収できるカードが1枚もなければ不発。
  ...
  runEffect: function (item, uiOps, done) {
    ...
    const ctx = {
      ...
      /* ★拾ったカードを画面の中央に見せる（v0.6.8）。
         カードを出したときと同じ演出です。

         二つの役目があります。
           ・公開すると書かれた効果（シルヴィ・アネット）では、
             相手にも見せる必要があるため
           ・トラッシュから拾う効果（案山子・引き戻す力）では、
             何を拾ったのか分かりやすくするため

         画面のない場所（AIの手番・検証）では何もしないので、
         道具が無いときは、そのまま次へ進みます。 */
      showCards: function (cards, next) {
        const list = (Array.isArray(cards) ? cards : [cards]).filter(Boolean);
        if (!uiOps.showCards || list.length === 0) { next(); return; }
        uiOps.showCards(list, next);
      },
    };
   ...
   */
  _addFromDeckToHand: function (side, card, label, reveal) {
    const st = this.state;
    const p = st.players[side];
    ...
    /* ★「公開し、手札に加える」効果は、相手にもカード名を見せます（v0.6.8）。
       見せないと、本当に条件に合うカードを取ったのか確かめられません。
       シルヴィとアネットが、これにあたります。 */
    if (reveal) {
      st.log.push((label || 'ドロー') + '：' + p.label + ' ' +
                  card.master.name + '（公開）');
    } else {
      this.logHidden(side,
   ...
   */
  resolveLook: function (side, looked, taken, reveal) {
    const st = this.state;
    const p = st.players[side];
    ...
    const self = this;
    takenList.forEach(function (c) {
      self._addFromDeckToHand(side, c, '回収', reveal);
    });
