...
const ENTER_EFFECTS = {
  ...
  village_kakashi: function (ctx, done) {
    ...
    ctx.pickCards({
      ...
      /* トラッシュは元から誰でも見られるので、公開の必要はありません。
         ただし★何を拾ったのかを分かりやすくするため、
         カードを出したときと同じように画面の中央へ見せます（v0.6.8）。 */
      if (chosen.length > 0) {
        ctx.game.moveTrashToHand(ctx.side, chosen[0]);
  ...
  mansion_sylvie: function (ctx, done) {
    ...
    pickGoods(function () {
      pickIsabella(function () {
        /* ★「公開し、手札に加える」なので、相手にも見せます（v0.6.8）。
           見せないと、本当に条件に合うカードを取ったのか確かめられません。 */
        ctx.game.resolveLook(ctx.side, looked, taken, true);
...
const LEAVE_EFFECTS = {
  ...
  mansion_annette: function (ctx, done) {
    ...
    ctx.pickCards({
      ...
      // ★「公開し、手札に加える」なので、相手にも見せます（v0.6.8）
      ctx.game.resolveLook(ctx.side, looked, chosen.length ? chosen[0] : null, true);
      ctx.showCards(chosen, done);
    });
  },
