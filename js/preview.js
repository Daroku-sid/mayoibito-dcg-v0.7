...
const view = {
  ...
  oppHandCount: 5,
  handExpanded: false,
  handMoveUntil: 0,       // 手札が動き終わる時刻（着地点の計算に使う）
  handSelected: -1,
};
...
function makeCard(spec) {
  ...
  // どのカードかを要素に残しておきます。
  // チュートリアルが「このカードを光らせて」と指示するときの目印です（v0.5）。
  if (spec.cardId) el.dataset.cardId = spec.cardId;
...
function attachCardInput(el, spec, place, zoneId) {
  attachPointer(el, {
    ...
    onTap: function () {
      /* ★演出中や相手のターン中でも、カードの詳細は見られます（v0.6.7）。
         盤面が動いているあいだ「あれは何だったのか」を確かめたくなるのは
         自然なことで、そこを止める理由がありません。

         ただし、盤面を動かす操作は今までどおり止めます。
         見るだけにして、ゲームの進行には触れません。 */
      if (view.locked && !boardPick) {
    ...
    onLongPress: function () {
      // ★拡大詳細も、演出中や相手のターン中に見られます（v0.6.7）
      openZoomDetail(spec);
    },
...
function fillZone(id, specs) {
  ...
  specs.forEach(function (spec) {
    /* ★グッズを装備しているなら、二枚を重ねて置きます（v0.6.7）。
       下にグッズ、その上に本体。グッズは下へ少しずらして、
       カード名のあたりがのぞくようにします。 */
    if (spec.goods) {
      const stack = document.createElement('div');
      stack.className = 'card-stack';

      const under = makeCard(spec.goods);
      under.classList.add('card--equipped');
      attachCardInput(under, spec.goods, 'board', id);
      stack.appendChild(under);

      const el = makeCard(spec);
      el.classList.add('card--holder');
...
function renderFan() {
  ...
  // マリガン中だけは開いたままにします。
  // 交換するカードを選ぶ場面なので、閉じると操作そのものができません。
  //
  // v0.3では「自分の番の自動処理中」も開いたままにしていましたが、
  // ドロー演出のために表示を縛るのはやめました（v0.4）。
  // 飛んでくるカードの行き先は reaimFlyingCards が合わせます。
  if (play.mode === 'mulligan') view.handExpanded = true;

  /* 簡略表示のとき。
     visibility ではなく opacity を使うのは、
     入れ替わりを薄くつなぐためです（v0.4.1）。 */
  if (!view.handExpanded) {
    fan.classList.add('is-hidden');
    miniSelf.style.opacity = '1';
    miniSelf.style.pointerEvents = '';
  ...
  /* 拡大中は簡略手札を隠します（同じ場所に重なるため）。
     ただし拡大手札に1枚も無いあいだは、簡略手札を出したままにします。
     どちらも見えない「手札が消えた」状態を作らないためです（v0.4.1）。 */
  const showMini = (view.hand.length === 0);
  miniSelf.style.visibility = 'visible';
  miniSelf.style.opacity = showMini ? '1' : '0';
  miniSelf.style.pointerEvents = showMini ? '' : 'none';
...
function setCandidate(youkai, human) {
  /* チュートリアル中は、台本が指した組み合わせだけを認めます（v0.5）。
     解除（両方 null）はいつでも通します。やり直せないと詰むためです。 */
  if (youkai && human) {
    const ok = TutorialActions.allow('selectPursuit', {
      cardId: youkai.cardId, targetId: human.cardId,
    });
    if (!ok) { renderCandidate(); return; }
  }
...
function playBanner(text, options, next) {
  const opt = options || {};
  // 対戦をやめたあとは、この続きを進めません（v0.4.4）
  const go = sessionGuard(next || function () {});
  view.locked = true;
  closeQuickDetail();
  ...
  setTimeout(function () {
    ...
    setTimeout(function () { go(); }, ms(STEP_GAP));
  }, ms(opt.hold || 900));
}
function hideBanner() {
...
function confirmTracking() {
  ...
  closeQuickDetail();                       // 2. クイック詳細を閉じる
  view.handSelected = -1;                   // 手札の拡大・簡略はその人の指示のまま
  renderCandidate();
...
const play = {
  ...
  session: 0,             // 対戦ごとの通し番号。やめた後の演出を打ち切るために使う
  handSnapshot: null,     // ドロー演出中に映しておく「増える前の手札」
  handSnapshotSide: null, // その控えが、どの席のものか

  /* ★まだ配っていない席は、手札を映しません（v0.6.7）。
     配り演出より先に手札が見えていると、
     「配られる前から持っている」ことになって不自然です。

     プレイヤーが後手のとき、相手のマリガンが先に走ります。
     そのあいだ自分の手札が丸見えになっていました。 */
  dealt: {},
...
function startGame(firstSide, seed, options) {
  ...
  play.session += 1;          // 前の対戦の演出が混ざらないようにする
  advancedSnapshotDone = false;   // 実践編の盤面は、対戦ごとに1回だけ作ります
  clearFlyingCards();

  /* 前の対戦のチュートリアル表示が残っていたら消します（v0.5.4）。
     始めるときにも消しておくと、片づけ漏れがあっても
     次の対戦には持ち越しません。 */
  if (typeof TutorialUI !== 'undefined' && !TutorialController.active) TutorialUI.clear();

  Game.start(firstSide || 'village', seed || '', options);

  /* チュートリアルなら、ここで手札と山札を台本どおりに並べ替えます（v0.5）。
     ルール処理には触れず、配り終わった盤面を外から整えるだけです。
     カードの増減は起きません（tutorial-deck.js の説明を参照）。 */
  if (TutorialController.active && TutorialController.data) {
    const data = TutorialController.data;

    const plan = data.openingPlan || {};
    Object.keys(plan).forEach(function (side) {
      const ok = TutorialDeck.apply(side, plan[side]);
      if (!ok) Errors.note('チュートリアル：初期手札を仕込めませんでした（' + side + '）');
    });

    /* ★実践編の盤面（仕様書 21.2）を、ここで作ります（v0.6.2）。
       説明が始まる前に作らないと、読んでいる裏で盤面が
       ぱっと入れ替わって見えます。

       このあとターン開始の処理で気力が1増えてカードを1枚引くので、
       そのあとにもう一度同じ形へ整えます（applyAdvancedSnapshot）。
       同じ指示を二度あてても結果は変わらない作りにしてあります。 */
    applyAdvancedSnapshot(true);
  }
  ...
  /* ★対戦の途中から始めるチュートリアル（実践編）は、
     マリガンを飛ばします（v0.6.1）。

     実践編は「試合の途中」という設定なので、
     手札を引き直す場面はもう終わっています。
     ここを飛ばさないと、盤面を組み立てる前に
     マリガンの案内が出てしまい、進行不能になります。 */
  if (TutorialController.active && TutorialController.data &&
      TutorialController.data.openingSnapshot) {
    skipMulliganForTutorial();
    return;
  }
...
/* =====================================================================
   マリガンを飛ばす（v0.6.1・実践編用）
   ---------------------------------------------------------------------
   両方の席とも「交換なし」で決着させ、そのまま第1ターンへ進みます。
   ルール処理は通常どおり通します（confirmMulligan を呼びます）。
   ===================================================================== */
function skipMulliganForTutorial() {
  const st = Game.state;
  // 配り演出をしないので、両席とも配り済み扱いにします（v0.6.7）
  play.dealt[st.firstSide] = true;
  play.dealt[st.secondSide] = true;
  Game.confirmMulligan(st.firstSide, []);
  Game.confirmMulligan(st.secondSide, []);
  play.mode = 'main';
  renderAll();
  finishMulligan();
}
...
function beginMulligan(side) {
  ...
  play.handSnapshotSide = side;      // この控えは、どの席のものか
  setHandExpanded(true, { skipRender: true });
  renderAll();

  // CPUの席は、手札を見せずに自動で済ませる（仕様書 14.2）
  if (isCpuSide(side)) {
    play.dealt[side] = true;      // 見せない席なので、配り済み扱いにします
    hideDim();
  ...
  playBanner(Game.labelOf(side) + 'のマリガン', { hold: 1200 }, function () {
    ...
    flyCardSequence(items, function () {
      play.handSnapshot = null;
      play.handSnapshotSide = null;
      play.dealt[side] = true;    // ★この席は配り終えました
...
function toggleMulligan(inst) {
  ...
  // チュートリアル中は、台本が指したカードだけを選べます（v0.5）
  if (!TutorialActions.allow('mulliganSelect', { cardId: inst.cardId })) return;
  ...
  // 指定の2枚がそろったら、次のステップへ進みます
  const chosen = play.mulliganSelected.map(function (uid) {
    const c = Game.state.players[play.mulliganSide].hand.find(function (h) {
      return h.uid === uid;
    });
...
function confirmMulliganNow() {
  ...
  // チュートリアル中は、台本がここまで進んでいるときだけ押せます（v0.5）
  if (!TutorialActions.allow('mulliganConfirm', {})) return;
...
function afterMulliganConfirmed(side, count) {
  ...
  /* チュートリアル：マリガンをすると山札が混ぜ直されるので、
     このあとの引き順をもう一度仕込みます（仕様書 20.2・v0.5）。 */
  if (TutorialController.active && TutorialController.data) {
    const plan = (TutorialController.data.afterMulliganPlan || {})[side];
    if (plan) {
      const ok = TutorialDeck.apply(side, plan);
      if (!ok) Errors.note('チュートリアル：交換後の手札を仕込めませんでした（' + side + '）');
    }
...
function syncFromGame() {
  ...
  /* ドロー演出の最中は「増える前の手札」を映しておき、
     ...
     ★ただし、その控えが「いま画面の下に出ている席」のものであるときだけです。
     相手のマリガン中は相手の控えが空のまま置かれているので、
     そのまま映すと自分の手札が消えてしまいます（v0.4.1で見つけた不具合）。 */
  const snapshotIsMine = play.handSnapshot &&
    (play.handSnapshotSide == null || play.handSnapshotSide === meSide);

  /* ★まだ配り演出をしていない席は、手札を映しません（v0.6.7）。
     後手のとき、相手のマリガン中に自分の手札が見えていました。 */
  if (play.active && !play.dealt[meSide]) {
    view.hand = [];
  } else {
    view.hand = snapshotIsMine ? play.handSnapshot.slice() : me.hand.slice();
  }
...
function renderPicker() {
  ...
  btn.addEventListener('click', function () {
    ...
    /* ★チュートリアル中は、台本が「このカードを選ぶ」と決めている場面で
       0枚のまま確定させません（v0.6.3）。
       たとえば《引き戻す力》でヌシ様を拾わないと、
       このあとヌシ様を出す場面へ進めなくなります。 */
    if (!TutorialActions.allow('pickConfirm', { count: n })) return;
...
function togglePick(inst) {
  ...
  /* ★チュートリアル中は、台本が指したカードだけ選べます（v0.6）。
     境界線で捨てる札、引き戻す力で回収する札は台本で決まっています。
     断るときは何も変えません。 */
  if (!TutorialActions.allow('pickCard', { cardId: inst.cardId })) return;
  const at = picker.chosen.indexOf(inst);
...
const uiOps = {
  confirmYesNo: function (title, message, cb) {
    flushEffectAnimations(function () {
      /* ★チュートリアル中は、説明を読み終えてからダイアログを出します（v0.5.5）。
         説明の裏でダイアログが答えられると、ゲームだけ進んで
         チュートリアルが取り残され、進行不能になります。
         showDialog は押した瞬間に閉じるので、
         「押されてから断る」やり方では手遅れなのです。 */
      TutorialActions.whenExplained(function () {
        showDialog({
          title: title,
          message: message,
          buttons: [
            { label: '発動しない', onClick: function () {
                // どちらを選んでも先へ進めます。行き止まりを作らないためです
                TutorialActions.notify('fieldSkipped', {});
                cb(false);
              } },
            { label: '発動する', primary: true, onClick: function () {
                TutorialActions.notify('fieldUsed', {});
                cb(true);
              } },
          ],
        });
  ...
  pickCards: function (options, cb) {
    /* 説明の裏で選択画面が出ると、読み終えた瞬間に
       何を選んだのか分からないまま進みます（v0.5.5と同じ理由）。 */
    flushEffectAnimations(function () {
      TutorialActions.whenExplained(function () { openCardPicker(options, cb); });
    });
  },
  /* ★拾ったカードを画面の中央に見せます（v0.6.8）。
     カードを出したときと同じ演出を使い回します。
     複数枚なら1枚ずつ順に見せます。 */
  showCards: function (cards, next) {
    const list = (cards || []).slice();
    if (list.length === 0) { next(); return; }

    const showOne = function (i) {
      if (i >= list.length) { next(); return; }
      CpuDriver.reveal(list[i], function () { showOne(i + 1); });
    };
    // 手札が増えたことを先に描いてから見せます
    flushEffectAnimations(function () { showOne(0); });
  },
...
function uiOpsFor(item) {
  if (isCpuSide(item.side)) {
    const ops = AiUiOps.create(match.ai[item.side], item);

    /* ★選ぶことはAIに任せますが、「拾ったカードを見せる」のは
       画面の仕事です（v0.6.9）。

       CPUがシルヴィやアネットを使ったとき、
       何を手に入れたのか分からない、という指摘がありました。
       AIの受け答えには画面がないので、ここで画面側のものを渡します。 */
    ops.showCards = uiOps.showCards;
    return ops;
  }
  return uiOps;
}
...
/** 待機している効果をすべて順に解決する */
function runPendingEffects(rawDone) {
...
/* =====================================================================
   手札の拡大・簡略の切り替え（v0.4 で作り直し）
   ---------------------------------------------------------------------
   v0.3では「ドローが起きるかもしれない場面では、手札を必ず拡大表示に
   しておく」という作りにしていました。飛んできたカードが
   何も無いところへ着地しないようにするためです。

   ただ、これだと閉じたいときに閉じられず、不自然でした。
   v0.4では、いつでも自由に切り替えられるようにします。

   そのかわり、カードが飛んでいる最中に切り替えられたときは、
   飛んでいるカードの行き先を、新しい手札の位置へ向け直します
   （下の reaimFlyingCards）。
   ===================================================================== */
function setHandExpanded(on, opts) {
  const o = opts || {};
  if (view.handExpanded === on) return;

  view.handExpanded = on;
  view.handMoveUntil = nowMs() + HAND_MOVE_MS;   // 動いているあいだを控える
  if (!on) {
    view.handSelected = -1;
    if (!o.keepDetail) closeQuickDetail();
  }
  const check = document.getElementById('hand-expanded');
  if (check) check.checked = on;

  if (!o.skipRender) renderFan();

  /* 飛んでいるカードがあれば、行き先を新しい手札の位置へ向け直します。
     ここで直接呼ばず1回描画を待つのは、
     呼び出し側が renderAll() をこのあとで行うことがあるためです。
     画面が変わる前に位置を測ると、古い位置のまま向け直してしまいます。 */
  if (typeof requestAnimationFrame === 'function') {
    requestAnimationFrame(function () { reaimFlyingCards(); });
  } else {
    setTimeout(reaimFlyingCards, 16);
  }
  // 手札が動き終わってから、もう一度だけ狙い直します。
  // 動いている最中は大まかな位置しか分からないためです。
  setTimeout(reaimFlyingCards, HAND_MOVE_MS + 20);
}
...
function playCardInGame(spec, target) {
  ...
  /* チュートリアル中は、台本が指したカードだけを出せます（v0.5）。
     断るときは盤面を一切変えません。カードは自然に手札へ戻り、
     気力も減らず、ログにも残りません（仕様書 22.5）。 */
  /* ★どこへ置こうとしたかも伝えます（v0.6）。
     グッズは「誰に付けるか」まで台本で決まっているので、
     カードの種類だけを見ていると、別の相手へ付けられてしまいます。
       kind='equip' … 付ける相手のカードID
       kind='unit'  … 'unit'（人間・怪異のエリア）
       kind='event' … 'event'（イベント使用エリア） */
  const targetInst = (target.kind === 'equip' && target.el && target.el._spec)
    ? target.el._spec.inst : null;
  const targetId = (target.kind === 'equip')
    ? (targetInst ? targetInst.cardId : null)
    : target.kind;

  if (!TutorialActions.allow('playCard', { cardId: inst.cardId, targetId: targetId })) {
    renderAll();
    return;
  }
  ...
  // チュートリアル：このカードを出すのが台本の完了条件なら、次へ進みます（v0.5）
  TutorialActions.notify('cardPlayed', { cardId: inst.cardId });
...
function confirmTrackingFromMain() {
  ...
  // チュートリアル中は、台本がここまで進んでいるときだけ押せます（v0.5）
  if (!TutorialActions.allow('confirmPursuit', {})) return;

  /* 「まだプレイできるカードがあります」は通常は親切ですが、
     台本どおりに進めようとする手を止めてしまいます（仕様書 24）。 */
  const stillPlayable = Game.hasMeaningfulPlay(side) &&
                        TutorialActions.showsWarning('playableCardWarning');
...
function endTurnFlow() {
  ...
  // チュートリアル中は、台本がここまで進んでいるときだけ終われます（v0.5）
  if (!TutorialActions.allow('endTurn', {})) return;

  /* ★「ターンが終わった」という合図は、ここでは出しません（v0.6.4）。
     このあと確認ダイアログが出ることがあり、
     そこで「戻る」を選べばターンは終わらないからです。

     押した瞬間に出していたため、
       ・確認で戻ってもターンが終わったことになる
       ・確認を待つステップが、先に出た合図を取り逃がして進めない
     という不具合が起きていました。

     実際に終わる doEndTurn で出します。 */
  ...
  /* チュートリアル中は、台本を妨げる警告を抑えます（仕様書 24）。
     抑えるのは文言だけで、判定そのものは通常どおりです。 */
  if (me.youkai.length > 0 && opp.humans.length > 0 &&
      TutorialActions.showsWarning('noPursuitWarning')) {
  ...
  /* 警告が出たことを台本へ伝えます（v0.6）。
     実践編では、気力を残してターンを終える場面で
     わざとこの警告を見せます（仕様書 21.6）。 */
  TutorialActions.notify('endTurnWarning', {});
  ...
  TutorialActions.whenExplained(function () {
    showDialog({
      ...
      buttons: [
        { label: '戻る' },
        {
          label: 'ターンを終了', primary: true,
          onClick: function () {
            // 台本がここまで来ているときだけ確定できます（v0.6.4）
            if (!TutorialActions.allow('confirmEndTurn', {})) return;
            TutorialActions.notify('confirmEndTurn', {});
            doEndTurn();
          },
        },
      ],
...
/**
 * ターンを実際に終える。
 * byPlayer が true なら、あなたの操作で終えたということです（v0.6.4）。
 * CPUの台本から呼ぶときは合図を出しません。
 */
function doEndTurn(byPlayer) {
  closeQuickDetail();
  view.handSelected = -1;

  // ★ここで初めて「ターンが終わった」と伝えます
  if (byPlayer !== false) TutorialActions.notify('turnEnded', {});
...
function confirmTrackingInGame() {
  ...
  // チュートリアル：確定が台本の完了条件なら、次のステップへ（v0.5）
  TutorialActions.notify('pursuitConfirmed', {});
...
function autoEndTurn() {
  ...
  // クイック詳細と選択を解きます。
  // 手札の拡大・簡略はその人が決めた形のままにします（v0.4.1）。
  view.handSelected = -1;
  setCandidate(null, null);
  closeQuickDetail();
  renderAll();
...
function switchBoardTo(side) {
  ...
  // ターン開始時は選択を解きます。
  // 拡大・簡略の状態は、その人が決めた形をそのまま引き継ぎます（v0.4）。
  view.handSelected = -1;

  /* ★盤面が上下入れ替わるので、開いている詳細は閉じます（v0.6.7）。
     ひとり回しは端末を渡して交代するため、
     前の人が見ていたカードが残っていると不自然です。
     CPU対戦は盤面が入れ替わらないので、ここは通りません
     （switchBoardTo を呼ぶのは、入れ替わる場面だけです）。 */
  closeQuickDetail();
  closeZoomDetail();
...
function runTurnStart(side) {
  ...
  /* チュートリアル中は、襲撃の説明を読み終えてから始めます（v0.5.2）。
     ここで待たないと、説明ウィンドウの裏で襲撃が終わってしまい、
...
/* =====================================================================
   襲撃の演出（仕様書 21.4・v0.4.3 で作り直し）
   ---------------------------------------------------------------------
   怪異が持ち上がって人間へぶつかり、ぶつかった所で光が弾け、
   体力の数字が赤くなってから減る、という流れにします。
...
/* =====================================================================
   対戦をやめたあとの演出を打ち切る（v0.4.4）
   ---------------------------------------------------------------------
   演出は setTimeout で先の処理を予約しながら進みます。
   リタイアしても、その予約は残ったままなので、
   画面の裏で対戦が進み続けていました。

   対戦ごとに通し番号を振り、予約したときの番号と食い違えば
   何もしないようにします。
   ===================================================================== */
/** 飛んでいる途中のカードを片づける */
function clearFlyingCards() {
  flyingCards.forEach(function (f) {
    if (f.anim) { try { f.anim.cancel(); } catch (e) { /* 無視 */ } }
    if (f.el) f.el.remove();
  });
  flyingCards = [];
  ['fly-layer', 'fly-layer-top'].forEach(function (id) {
    const el = document.getElementById(id);
    if (el) el.innerHTML = '';
  });
}

function sessionGuard(fn) {
  const mine = play.session;
  return function () {
    if (!play.active || play.session !== mine) return undefined;
...
/** いまの対戦がまだ続いているか */
function matchAlive() {
  return play.active && !!Game.state;
}

/**
 * 対戦していないあいだは盤面を隠す（v0.4.4）。
 * メニューの背景は半透明なので、そのままだと
 * 前の対戦の盤面がうっすら透けて見えてしまいます。
 */
function applyBoardVisibility() {
  document.body.classList.toggle('no-match', !play.active);
}

/** 盤面にある、そのカードの要素を探す */
function boardCardEl(inst) {
  if (!inst || !inst.uid) return null;
  return document.querySelector('.card[data-uid="' + inst.uid + '"]');
}

/**
 * 怪異が人間へぶつかりに行く動き。
 * 相手の位置まで寄っていき、ぶつかった瞬間に戻ります。
 * @param done ぶつかった瞬間に呼ばれる
 */
function lungeAttacker(attackerEl, defenderEl, done) {
  const finish = done || function () {};
  if (!attackerEl || !defenderEl || !attackerEl.animate) { finish(); return; }

  const a = designRect(attackerEl);
  const d = designRect(defenderEl);
  // 相手の手前で止めます。重なりきると、どちらが殴ったのか分かりません
  const dx = (d.x + d.w / 2) - (a.x + a.w / 2);
  const dy = (d.y + d.h / 2) - (a.y + a.h / 2);
  const reach = 0.62;

  /* ★重なりの対策（v0.4.4）
     カードは陣地ごとの入れ物（.zone）に入っています。
     カードだけ前に出しても、入れ物どうしの前後は変わらないので、
     相手の怪異が自分の人間の下へ潜り込んで見えていました。
     入れ物ごと前に出します。 */
  const zone = attackerEl.parentElement;
  const zoneZ = zone ? zone.style.zIndex : '';
  if (zone) zone.style.zIndex = '30';
  attackerEl.style.zIndex = '5';

  const anim = attackerEl.animate([
    { transform: 'translate(0,0) scale(1)' },
    { transform: 'translate(' + (-dx * 0.12) + 'px,' + (-dy * 0.12) + 'px) scale(1.06)', offset: 0.28 },
    { transform: 'translate(' + (dx * reach) + 'px,' + (dy * reach) + 'px) scale(1.12)', offset: 0.62 },
    { transform: 'translate(' + (dx * reach * 0.9) + 'px,' + (dy * reach * 0.9) + 'px) scale(1.08)', offset: 0.72 },
    { transform: 'translate(0,0) scale(1)' },
  ], { duration: ms(760), easing: 'cubic-bezier(0.3, 0, 0.2, 1)' });

  anim.onfinish = function () {
    attackerEl.style.zIndex = '';
    if (zone) zone.style.zIndex = zoneZ;
  };
  // ぶつかる瞬間（62%あたり）で知らせる
  setTimeout(finish, ms(Math.round(760 * 0.62)));
}
...
function burstAt(el) {
  ...
  fx.style.left = (r.x + r.w / 2) + 'px';
  fx.style.top = (r.y + r.h / 2) + 'px';
  const layer = document.getElementById('fly-layer-top') || document.getElementById('stage');
  layer.appendChild(fx);

  if (el.animate) {
    el.animate([
      { filter: 'brightness(2.4)', transform: 'translate(0,0)' },
      { filter: 'brightness(1.2)', transform: 'translate(-7px, 3px)', offset: 0.3 },
      { filter: 'brightness(1.4)', transform: 'translate(6px, -2px)', offset: 0.6 },
      { filter: 'brightness(1)', transform: 'translate(0,0)' },
    ], { duration: ms(420), easing: 'ease-out' });
  }
  setTimeout(function () { fx.remove(); }, ms(560));
}

/** 襲撃の演出（仕様書 21.4） */
function playAttack(info, rawDone) {
  const done = sessionGuard(rawDone || function () {});
  view.locked = true;
  closeQuickDetail();
  ...
  const attackerEl = boardCardEl(info.attacker);
  const defenderEl = boardCardEl(info.defender);

  // 0. 怪異が持ち上がり、人間へぶつかりに行く
  lungeAttacker(attackerEl, defenderEl, function () {
    Se.play('damage');
    burstAt(defenderEl);            // ぶつかった所で光が弾ける
    burstAt(attackerEl);            // 反撃を受けるので、こちらも揺れる

    // 体力の数字を先に赤くしておく（減る前に「ここが減る」と分かる）
    [defenderEl, attackerEl].forEach(function (el) {
      const hp = el && el.querySelector('.ov--hp');
      if (hp) hp.classList.add('is-hit');
    });
  });
  ...
  setTimeout(function () {
    ...
    renderAll();
    // 描き直したあとの数字を、赤いまま少し残す
    [info.defender, info.attacker].forEach(function (inst) {
      const el = boardCardEl(inst);
      const hp = el && el.querySelector('.ov--hp');
      if (!hp) return;
      hp.classList.add('is-hit');
      setTimeout(function () { hp.classList.remove('is-hit'); }, ms(520));
    });
    ...
    setTimeout(function () {  /* 演出の間隔 */
      ...
      animateZoneChanges(bottomSide(), beforeSelf, function () {
        animateZoneChanges(topSide(), beforeOpp, function () {
          // チュートリアル：襲撃の結果説明へ進む（v0.5）
          TutorialActions.notify('assaultDone', {});
          setTimeout(done, ms(320));
        });
      });
    }, ms(620));
  }, ms(760));   // 怪異がぶつかり終わるのを待ってから数字を動かす
}
...
function afterAttack(side) {
  ...
  runPendingEffects(function () {
    ...
    /* ★実践編は対戦の途中から始まります（仕様書 21.2）。
       気力を足してカードを引いたあとの、この時点で盤面を組み立てます。
       先に作ってしまうと、そのあとの気力とドローで数が狂います。 */
    applyAdvancedSnapshot();
    ...
    animateZoneChanges(side, before, function () {
      ...
      playBanner(Game.labelOf(side) + '\nのメインフェイズ', { hold: 900 }, function () {
        ...
        /* チュートリアル中は、通常のAIではなく台本で動かします（仕様書 19.1）。
           画面の難易度表示は「弱」のままですが、
           重要な行動は決め打ちにしないと説明と食い違います。 */
        if (TutorialController.active && isCpuSide(side)) {
          runTutorialCpuTurn(side);
          return;
        }
...
/* =====================================================================
   チュートリアル用の入口（v0.5）
   ---------------------------------------------------------------------
   選択画面から呼ばれます。通常の対戦開始と同じ道を通しますが、
   手札の仕込みだけが startGame の中で足されます。
   ===================================================================== */
window.startTutorialGame = function (opts) {
  /* ★対戦画面は「画面のひとつ」ではありません。
     メニューの層（#start-screen）を閉じると、その下から現れます。
     'play' という名前の画面は存在しないので、そこへ飛ばそうとすると
     メニューが開いたままになり、盤面が見えません（v0.5.0の不具合）。

     あわせて、カード画像の先読みも通常の対戦と同じように通します。
     先読みを飛ばすと、絵が出そろわないまま対戦が始まります。 */
  Screens.close();

  Screens.startWithLoading([opts.decks.village, opts.decks.mansion], function () {
    startGame(opts.firstSide, opts.seed, {
      decks: opts.decks,
      labels: opts.labels,
      cpu: opts.cpu,
      tutorial: true,
...
/* =====================================================================
   実践編の開始盤面を組み立てる（v0.6・仕様書 21.2）
   ---------------------------------------------------------------------
   対戦の途中から始めるため、場・気力・追跡を外から作ります。
   一度だけ走ります。ルール処理には触れません。
   ===================================================================== */
let advancedSnapshotDone = false;

/**
 * first が true なら、対戦を始めた直後の1回目。
 * false なら、ターン開始の処理のあとの整え直し（1回だけ）。
 */
function applyAdvancedSnapshot(first) {
  if (!first && advancedSnapshotDone) return;
  if (!TutorialController.active || !TutorialController.data) return;

  const snap = TutorialController.data.openingSnapshot;
  if (!snap) return;
  if (!first) advancedSnapshotDone = true;

  ['village', 'mansion'].forEach(function (side) {
    if (!snap[side]) return;
    if (!TutorialDeck.applySnapshot(side, snap[side])) {
      Errors.note('チュートリアル：盤面を作れませんでした（' + side + '）');
    }
  });

  if (snap.tracking) {
    const t = snap.tracking;
    if (!TutorialDeck.setTracking(t.side, t.youkai, t.targetSide, t.human)) {
      Errors.note('チュートリアル：追跡の関係を作れませんでした');
    }
  }

  /* 組み立てた結果が台本どおりか、その場で点検します。
     ずれたまま進むと、説明と盤面が食い違います。 */
  ['village', 'mansion'].forEach(function (side) {
    if (!snap[side]) return;
    const problems = TutorialDeck.verifySnapshot(side, snap[side]);
    if (problems.length) {
      Errors.note('チュートリアル：盤面がずれています（' + side + '）' + problems.join(' / '));
    }
  });
...
/* =====================================================================
   相手の行動の説明を出す（v0.5.4・仕様書 20.4）
   ---------------------------------------------------------------------
   中央のダイアログではなく、画面の下に寄せて出します。
   相手が何をしたのかを、盤面を見ながら読めるようにするためです。
   ===================================================================== */
function showTutorialSay(text, next) {
  const box = document.getElementById('tut-say');
  if (!box) { next(); return; }

  box.querySelector('.tut-say__text').textContent = text;
  box.classList.remove('is-hidden');

  const btn = document.getElementById('tut-say-next');
...
function hideTutorialSay() {
  const box = document.getElementById('tut-say');
  if (box) box.classList.add('is-hidden');
}

/* =====================================================================
   チュートリアルを終えてメニューへ戻る（v0.5.3）
   ---------------------------------------------------------------------
   ★対戦から抜けるには、後片づけが要ります。
   これを省くと、対戦が動いたままメニューが開き、
   ボタンが二重に反応するなど、おかしな挙動になります（v0.5.2の不具合）。

   通常のリタイアと同じ backToSetupScreen を通し、
   戻り先だけをチュートリアル選択画面にします。
   ===================================================================== */
window.exitTutorialToMenu = function () {
  closeGameMenu();
  closeResultScreen();
  backToSetupScreen(null);        // 対戦の後片づけ一式（CPU停止・演出破棄など）
  Screens.goNow('tutorial-select');
  Screens.riseIn();
};

/* =====================================================================
   CPUの手番を台本どおりに進める（仕様書 20.4）
   ---------------------------------------------------------------------
   1手ごとに止めて説明を読んでもらいます。一括では実行しません。
   ここでは「台本の指示を、本物のゲーム処理へ翻訳する」ことだけをします。
   ===================================================================== */
function runTutorialCpuTurn(side) {
  view.locked = true;

  /** 場と手札から、指定のカードを探す */
  const findInHand = function (s, cardId) {
    return Game.state.players[s].hand.find(function (c) { return c.cardId === cardId; });
  };
  const findOnBoard = function (s, cardId) {
    const p = Game.state.players[s];
    return p.youkai.concat(p.humans).find(function (c) { return c.cardId === cardId; });
  };

  TutorialCpuDriver.start({
    /* ★カードの登場は、通常のCPU対戦と同じ演出に乗せます（v0.5.4）。
       一時公開 → 盤面へ出す → 効果の解決、まで終わってから
       done() を呼びます。説明はそのあとに出るので、
       何が起きたのかを見てから読めます。 */
    play: function (s, cardId, zone, done) {
      const inst = findInHand(s, cardId);
      if (!inst) { done(false); return; }

      CpuDriver.reveal(inst, function () {
        const r = Game.playUnit(s, inst);
        if (!r || !r.ok) { done(false); return; }
        renderAll();
        runPendingEffects(function () {
          renderAll();
          // 出たカードが落ち着くまで、ひと呼吸おきます
          setTimeout(function () { done(true); }, ms(420));
        });
      });
    },

    pursue: function (s, cardId, targetId, done) {
      const youkai = findOnBoard(s, cardId);
      const human = findOnBoard(otherSide(s), targetId);
      if (!youkai || !human) { done(false); return; }

      if (Game.state.phase === 'main') Game.endMain();
      Game.setTracking(s, youkai, human);
      Se.play('pursuit');
      renderAll();

      // 追跡の線が引かれるのを見せてから説明します
      setTimeout(function () { done(true); }, ms(760));
    },

    confirm: function (s, done) {
      Game.queueEndTurnEffects(s);
      renderAll();
      setTimeout(function () { done(true); }, ms(260));
    },

    /* 1手ごとの説明。演出が終わってから出ます。
       画面の下寄せにして、盤面が隠れないようにしています。 */
    say: function (text, next) {
      showTutorialSay(text, next);
    },

    render: function () { renderAll(); },

    /* 台本どおりに動かせなかったとき。
       黙って進むと説明と盤面が食い違うので、はっきり知らせます。 */
    onError: function (action) {
      Errors.note('チュートリアル：CPUの台本を実行できません（' + action.action + '）');
      showDialog({
        title: 'チュートリアルを続けられません',
        message: '想定と違う盤面になりました。\nお手数ですが、最初からやり直してください。',
        buttons: [{ label: 'メニューへ', primary: true, onClick: function () {
          TutorialRunner.quit();
        } }],
      });
    },
  }, function () {
    // 台本の4手が終わった。ここでターンを終える
    TutorialActions.notify('cpuScriptDone', {});
    runPendingEffects(function () {
      Game.toEndPhase();
      doEndTurn(false);      // CPUの台本なので、あなたの操作の合図は出しません
    });
  });
}
...
function finishWithResult() {
  ...
  /* チュートリアル中は、通常の結果画面を出しません（v0.6）。
     台本が用意したまとめを見せてから、選択画面へ戻します。
     ここで結果画面を出すと、まとめの上に重なってしまいます。 */
  if (TutorialActions.isActive()) {
    Se.play('win');
    TutorialActions.notify('gameOver', {});
    return;
  }
...
function backToSetupScreen(setupName) {
  ...
  /* ★対戦を離れるときは、チュートリアルも必ず片づけます（v0.5.4）。
     ここは対戦から抜ける唯一の出口なので、
     リタイア・クリア・結果画面のどこから来ても通ります。

     片づけを個別の出口に書くと、必ずどれかで書き忘れます。
     実際 v0.5.3 では、リタイアしたあとも案内メッセージが
     画面に残り続けていました。 */
  if (typeof TutorialCpuDriver !== 'undefined') TutorialCpuDriver.stop();
  if (typeof TutorialController !== 'undefined' && TutorialController.active) {
    TutorialController.quit();
  }
  if (typeof TutorialUI !== 'undefined') TutorialUI.clear();
  // 通し番号を進めて、予約されている演出をすべて無効にする（v0.4.4）
  play.session += 1;
  play.active = false;
  play.mode = 'idle';
  clearFlyingCards();
...
function handPointFor(side) {
  ...
  for (let i = 0; i < list.length; i++) {
    // 拡大手札が隠れているときは、簡略手札のほうを使います
    if (list[i] === '#hand-fan' && !isHandFanVisible()) continue;
 ...
 *
 * ★注意（v0.4で直した点）
 *   手札を簡略表示にすると #hand-fan は display:none になりますが、
 *   中のカードの要素は残ったままです。
 *   隠れた要素の位置を測ると縦横ゼロの点が返るため、
 *   そのまま使うと画面の左上へ飛んでいってしまいます。
 *   大きさがあることを確かめてから使います。
 */
function handRightPointFor(side) {
  // 出し入れの最中は、1枚ごとの細かい位置を当てにしません。
  // 手札全体の落ち着き先を狙ったほうが、着地が安定します。
  if (side === bottomSide() && isHandFanVisible() && !isHandMoving()) {
    const cards = document.querySelectorAll('#hand-fan .fan-card');
    if (cards.length) {
      const r = designRect(cards[cards.length - 1]);
      if (r.w > 0 && r.h > 0) return { x: r.x + r.w / 2, y: r.y + r.h / 2 };
    }
  }
...
/**
 * いま「何かを選んでいる最中」か。
 * 選択画面が開いている、盤面から選ばせている、効果の解決中などを指します。
 */
function isChoosingSomething() {
  const ids = ['picker', 'target-hint', 'dialog', 'sheet', 'zoom-detail', 'event-drop'];
  for (let i = 0; i < ids.length; i++) {
    const el = document.getElementById(ids[i]);
    if (el && (el.classList.contains('is-open') || el.classList.contains('is-on'))) return true;
  }
  return false;
}

/* 手札の出し入れにかかる時間（CSSの transition と合わせる） */
const HAND_MOVE_MS = 240;
...
function isHandMoving() {
  return nowMs() < (view.handMoveUntil || 0);
}

/**
 * 動きの影響を受けない位置を測る。
 * ---------------------------------------------------------------------
 * getBoundingClientRect は、親に掛かっている transform をそのまま含みます。
 * 手札の出し入れは transform で動かしているので、演出の途中に測ると
 * 「いま滑っている途中の位置」が返り、着地点がずれます。
 *
 * offsetLeft / offsetTop は transform を含まないので、
 * 動いていても「落ち着いたときの位置」が分かります。
 */
...
/** 拡大手札がいま画面に出ているか */
function isHandFanVisible() {
  const fan = document.getElementById('hand-fan');
  if (!fan || fan.classList.contains('is-hidden')) return false;
  const r = designRect(fan);
  return r.w > 0 && r.h > 0;
}
...
/* ★この割合まで進んだら、着いたものとして手札に反映します。
   0.82 だと、光が消えてから手札が浮かび上がっていました（v0.6.9）。
   光の到着に、手札の出現を間に合わせます。 */
const ARRIVE_RATIO = 0.62;
 ...
 */
/* いま飛んでいるカード。手札の表示が切り替わったとき、行き先を直すために持ちます */
let flyingCards = [];

/**
 * 飛んでいるカードの行き先を、いまの手札の位置へ向け直す（v0.4）。
 * ---------------------------------------------------------------------
 * 手札の拡大・簡略を切り替えると、着地するべき場所が動きます。
 * 出発したときの位置のまま飛ばすと、何も無いところへ着いてしまいます。
 *
 * いま見えている位置から、新しい行き先まで、残り時間で飛び直させます。
 * 途中で切り替えても、そのまま自然につながって見えます。
 */
function reaimFlyingCards() {
  if (!flyingCards.length) return;

  const now = (typeof performance !== 'undefined' && performance.now)
    ? performance.now() : Date.now();

  flyingCards.forEach(function (f) {
    if (f.done || !f.el.isConnected) return;
    if (typeof f.toFn !== 'function') return;   // 行き先が固定のものは触らない

    const goal = f.toFn();
    if (!goal) return;
    // ほとんど動いていなければ、飛び直す必要はありません
    if (Math.abs(goal.x - f.goal.x) < 2 && Math.abs(goal.y - f.goal.y) < 2) return;

    const left = Math.max(60, f.endAt - now);   // 残り時間。短すぎると瞬間移動に見える

    // いま光が見えている位置を、そのまま新しい出発点にする
    const r = designRect(f.el);
    const here = { x: r.x + r.w / 2, y: r.y + r.h / 2 };
    ...
    if (f.el.animate) {
      const ctrl = arcControlPoint(f.kind, here, goal);
      f.anim = f.el.animate(arcKeyframes(here, ctrl, goal),
        { duration: left, easing: 'ease-out' });
      f.anim.onfinish = function () { f.el.remove(); };
    }
    f.goal = goal;
  });
}
...
function flyCardSequence(items, done) {
  const finish = sessionGuard(done || function () {});
  ...
  items.forEach(function (item, index) {
    setTimeout(function () {
      ...
      const dur = ms(FLY_DURATION);
      const now = (typeof performance !== 'undefined' && performance.now)
        ? performance.now() : Date.now();

      // 行き先を直せるように控えておく
      entry = {
        el: el, kind: item.kind, goal: goal, endAt: now + dur,
        toFn: (typeof item.to === 'function') ? item.to : null,
        anim: null, done: false,
      };
      flyingCards.push(entry);

      if (el.animate) {
        const ctrl = arcControlPoint(item.kind, start, goal);
        entry.anim = el.animate(arcKeyframes(start, ctrl, goal),
                                { duration: dur, easing: 'cubic-bezier(0.35, 0, 0.3, 1)' });
        entry.anim.onfinish = function () { el.remove(); };
        setTimeout(function () { el.remove(); }, dur + 400);   // 念のための保険
      } else {
        setTimeout(function () { el.remove(); }, dur);
      }

      // 控えから外す（残り続けると、次の切り替えで無駄に走ります）
      setTimeout(function () {
        flyingCards = flyingCards.filter(function (f) { return f !== entry; });
      }, dur + 500);
...
function flashNewHandCard(inst) {
  ...
  /* ★光が着く瞬間には、もう見えている状態にします（v0.6.9）。
     以前は透明から始めて260ミリ秒かけていたので、
     光が消えたあとに手札が現れて見えました。
...
function openGameMenu() {
  ...
  /* チュートリアルの表示は重ね順が上なので、
     開いているあいだは引っこめます（v0.5.5）。
     そうしないとメニューが裏に隠れて押せません。 */
  if (typeof TutorialUI !== 'undefined') TutorialUI.suspend();
...
function closeGameMenu() {
  ...
  // 引っこめていたチュートリアルの表示を戻します（v0.5.5）
  if (typeof TutorialUI !== 'undefined') TutorialUI.resume();
...
function renderAll() {
  // ★出ているべきでないチュートリアル表示が残っていないか見張る（v0.5.4）
  if (typeof TutorialUI !== 'undefined') TutorialUI.audit();
...
function init() {
  ...
  applyBoardVisibility();   // 起動直後は対戦していないので盤面を隠す
  ...
  DeckPickerUI.build();                                 // 対戦で使うデッキ（仕様書 17）
  ...
  attachPointer(document.getElementById('self-hand-mini'), {
    onTap: function () {
      // 演出中でも切り替えられます（v0.4）。
      // 飛んでいるカードは reaimFlyingCards が行き先を合わせます。
  ...
  document.getElementById('stage').addEventListener('pointerup', function (e) {
    ...
    if (e.target.closest('.card, .ui-box, #event-drop, #quick-detail, #zoom-detail, ' +
                         ...
                         /* チュートリアルの表示も「空白」ではありません（v0.5.2）。
                            ここが抜けていたため、説明の「次へ」を押した指が
                            空白タップとみなされ、選んでいた追跡が解除されて
                            「追跡を確定」ボタンごと消えていました。 */
                         '#tut-panel, #tut-guide, #tut-hint')) return;

    /* チュートリアル中は、そもそも追跡を勝手に解除しません（v0.5.2）。
       台本が「この組み合わせで追跡してください」と指示したあとに
       解除されると、進行不能になります。 */
    if (typeof TutorialController !== 'undefined' && TutorialController.active) {
      closeQuickDetail();
      return;
    ...
    /* 何かを選んでいる最中は、空白のつもりのタップでも
       「選ぶ操作の一部」であることがあります。
       ここで手札を畳むと、選択画面の裏で勝手に形が変わって見えます
       （境界線などで対象を選ぶとき：v0.4.1で見つけた不具合）。 */
    if (isChoosingSomething()) return;
    closeQuickDetail();
