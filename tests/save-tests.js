...
console.log('\n■ ★チュートリアルのクリアが保存される（v0.5.3の不具合）');
{
  /* v0.5.2 では SaveManager.get('tutorial') で読もうとしていました。
     get/set は「設定」の欄だけを見るので、いつも undefined が返り、
     クリアしても実践編が解放されませんでした。 */
  const store = makeStore();
  const ctx = vm.createContext({
    window: { localStorage: store }, JSON, console, Date, Math, isFinite,
    CARD_MASTER: G.CARD_MASTER, APP_VERSION: '0.5.3',
    document: undefined, MutationObserver: undefined,
  });
  vm.runInContext(fs.readFileSync('js/save-manager.js', 'utf8'), ctx);
  vm.runInContext(fs.readFileSync('js/tutorial-runner.js', 'utf8'), ctx);
  const S = vm.runInContext('SaveManager', ctx);
  const R = vm.runInContext('TutorialRunner', ctx);

  S.load();
  check('最初は未クリア', R.isBasicCleared() === false);
  check('クリアを保存できる', R.saveBasicCleared() === true);
  check('保存直後は「クリア済み」', R.isBasicCleared() === true);

  S.data = null;
  S.load();
  check('★読み直してもクリアが残っている（ここが本体）',
    R.isBasicCleared() === true);
  check('保存先はチュートリアルの枠',
    S.data.tutorial && S.data.tutorial.basicCompleted === true);
  check('設定の欄を汚していない', S.data.settings.tutorial === undefined);
}
