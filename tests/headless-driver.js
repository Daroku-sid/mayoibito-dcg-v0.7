...
function playGameHeadless(G, opts) {
  ...
  /* AIは外から渡すこともできます（opts.ais）。
     新しいAIと前のAIを直接戦わせて強さを比べるために使います。
     渡さなければ、これまでどおり難易度から作ります。 */
  const ais = opts.ais || {
  ...
  ['village', 'mansion'].forEach(function (s) {
    /* 1枚ずつ選べるので、AIが選んだ札だけを戻します。
       cpu-driver.js と同じやり方にそろえています。
       ここが古いままだと、マリガンの賢さを測れません（v0.5.9で修正）。 */
    const ai = ais[s];
    Game.confirmMulligan(s,
      ai.chooseMulligan ? ai.chooseMulligan()
        : (ai.shouldMulligan() ? Game.state.players[s].hand.map(function (c) { return c.uid; }) : []));
  });
