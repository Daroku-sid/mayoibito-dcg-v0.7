...
function boot() {
  ...
  const ctx = vm.createContext({
    ...
    /* 対戦へ自作デッキを預ける入れもの。
       これが無いと prepareForBattle を一度も試せません（v0.5.8で追加）。 */
    RuntimeDecks: { custom: {}, set(id, def) { this.custom[id] = def; },
                    clear() { this.custom = {}; },
                    get(id) { return G.DECKS[id] || this.custom[id] || null; } },
  });
  ...
  return {
    ...
    R: vm.runInContext('RuntimeDecks', ctx),
  };
}
...
console.log('\n■ ★対戦で使うデッキが取り違わらない（v0.5.8の重大な不具合）');
{
  /* 対戦設定の画面は、公式デッキを 'village' / 'mansion' という
     名前で持っています。いっぽう DeckManager が使う id は
     'official_village' の形です。

     prepareForBattle は、id が見つからないときに
     「席の名前」へ落としていました。
     そのため CPU にヨマモリ村を選んでも、
     CPUは mansion 席なので黒薔薇の館になってしまっていました。 */
  const { S, D, C, R } = boot();

  const combos = [
    ['village', 'mansion'], ['mansion', 'village'],
    ['village', 'village'], ['mansion', 'mansion'],
  ];
  combos.forEach(function (pair) {
    const out = D.prepareForBattle({ village: pair[0], mansion: pair[1] });
    check('★あなた=' + pair[0] + ' / CPU=' + pair[1] + ' が入れ替わらない',
      out.village === pair[0] && out.mansion === pair[1],
      JSON.stringify(out));
  });

  /* 自作デッキと公式デッキの組み合わせでも取り違えないこと */
  const made = D.create(D.byId('official_village'), 'テスト自作').deck;
  check('自作デッキを作れた', !!made, made && made.id);

  ['village', 'mansion'].forEach(function (cpu) {
    const out = D.prepareForBattle({ village: made.id, mansion: cpu });
    check('★あなた=自作 / CPU=' + cpu + ' が入れ替わらない',
      out.village === made.id && out.mansion === cpu,
      JSON.stringify(out));
  });

  /* 自作デッキの中身は、対戦側へ預けられていること */
  const out = D.prepareForBattle({ village: made.id, mansion: 'village' });
  check('自作デッキの中身を対戦へ預けている',
    !!R.custom[made.id]);
  check('公式デッキは預けない（decks.js をそのまま使う）',
    Object.keys(R.custom).length === 1,
    Object.keys(R.custom).join('、'));
}
