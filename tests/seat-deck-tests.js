...
console.log('\n■ ★対戦を始める3つの道すじで、デッキが取り違わらない（v0.5.8）');
{
  const fs2 = require('fs');
  const src = fs2.readFileSync('js/screens.js', 'utf8');
  const dm = fs2.readFileSync('js/deck-manager.js', 'utf8');

  /* 対戦設定の画面は公式デッキを 'village' / 'mansion' という名前で持ちます。
     DeckManager の id は 'official_village' の形なので、
     そのままでは byId で見つかりません。
     見つからないときに席の名前へ落としていたため、
     CPUにヨマモリ村を選んでも黒薔薇の館になっていました。 */
  check('★公式デッキの名前をそのまま受け取れる',
    /DECKS\[id\]\)\s*\{\s*out\[seat\]\s*=\s*id;/.test(dm),
    "'village' のような名前で渡ってきても取り違えません");

  /* 対戦を始める道すじは3つ。どれも席への割り当て方が同じか見ます。 */
  const paths = [
    ['ひとり回し', /village:\s*s\.deck1,\s*mansion:\s*s\.deck2/],
    ['CPU対戦',   /village:\s*c\.playerDeck,\s*mansion:\s*cpuDeck/],
    ['CPU観戦',   /village:\s*w\.deck1,\s*mansion:\s*w\.deck2/],
  ];
  paths.forEach(function (p) {
    check(p[0] + '：席1＝village／席2＝mansion で渡している', p[1].test(src));
  });

  check('CPU対戦は席 village があなた、席 mansion がCPU',
    /village:\s*'あなた',\s*mansion:\s*'CPU'/.test(src));
}
