...
console.log('\n■ モード選択（仕様書 6・30.1）');
{
  ...
  /* v0.5.6 で、設定ボタンをモード選択の外へ出しました。
     画面が浮かび上がる演出は .menu に transform をかけます。
     transform のかかった要素は絶対配置の基準になるので、
     中に置いたままだと演出の前後で位置が飛びます。 */
  check('設定が端の固定位置にある（仕様書 6.1）',
    html.indexOf('id="corner-options"') !== -1 &&
    html.indexOf('data-go="options"') !== -1);
  check('★設定ボタンは演出のかかる枠の外にある',
    b.indexOf('menu__corner') === -1,
    '中に置くと、浮かび上がる演出の前後で位置が飛びます');
...
console.log('\n■ 対戦モード選択（仕様書 7・30.1）');
{
  ...
  // v0.5で実装したので「準備中」ではなく、選択画面へ進むようになりました
  check('チュートリアルが選択画面へ進む', b.indexOf('data-go="tutorial-select"') !== -1);
  check('戻るがある', b.indexOf('data-back') !== -1);
}
...
{
  const pv = fs.readFileSync('js/preview.js', 'utf8');
  /* v0.4.5 から、続けて何画面も積むところは goNow（幕なし）を使います。
     1画面ごとに幕を出すと、戻るだけで何秒も待たされるためです。 */
  check('スタート画面まで戻さず、対戦の階層へ戻す',
    /Screens\.reset\('mode'\);\s*\n\s*Screens\.goNow\('battle-mode'\);/.test(pv));
  check('観戦から戻ると開発者用モードを経由する',
    /Screens\.goNow\('dev-mode'\);\s*\n\s*Screens\.goNow\('watch-setup'\);/.test(pv));
  check('戻ったあとに黒から浮かび上がる',
    (pv.match(/Screens\.riseIn\(\);/g) || []).length >= 2);
}
