...
const secs = ['start','mode','battle-mode','card-mode','dev-mode',
              'deck-list','deck-view','deck-edit','field-select','deck-pick','card-list','cpu-setup','solo-setup','watch-setup','howto','options']
...
const S = vm.runInContext(`/* v0.4 で screens.js が使うようになったモジュールの、最小の代役。
   ここでは画面遷移だけを見たいので、中身は動けばよい。 */
const DeckPickerUI = { build(){}, render(){}, refreshLabels(){},
                       ensureUsable(v){ return v; } };
const DeckManager = {
  prepareForBattle(picks){ return { village: picks.village, mansion: picks.mansion }; },
};
const DeckEditorUI = { build(){}, render(){} };
const FieldPickerUI = { render(){} };
const DeckListUI = { build(){}, renderList(){}, renderView(){} };
const CardListUI = { build(){}, render(){} };
` + fs.readFileSync('js/screens.js', 'utf8') + '\n;Screens', ctx);
