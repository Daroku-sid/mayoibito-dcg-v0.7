...
/* v0.7 では器の CSS（v07.css）が加わりました。クラス定義の照合は
   両方をつないで見ます（既存 layout.css には触れていません）。 */
const css = fs.readFileSync('css/layout.css', 'utf8')
  + '\n' + fs.readFileSync('css/v07.css', 'utf8');
...
console.log('■ 版の番号がそろっている（仕様書 31・32.9）');
{
  ...
  check('v0.7 の器 CSS も読み込んでいる', /href="css\/v07\.css\?v=/.test(html));
...
console.log('\n■ 読み込みの取りこぼしがない');
{
  const loaded = (html.match(/src="js\/([a-z0-9\-]+\.js)/g) || [])
