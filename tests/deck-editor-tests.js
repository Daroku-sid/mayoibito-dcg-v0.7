...
console.log('\n■ ★見た目の決めごと（v0.5.7）');
{
  ...
  const src = fs2.readFileSync('js/deck-editor-ui.js', 'utf8');
  const css = fs2.readFileSync('css/layout.css', 'utf8');

  /* 主人公の印は「主」の一文字をやめ、錠前のマークにしました。
     形そのものが「外せない」を表すので、字を読まずに伝わります。 */
  check('★「主」の文字を出していない', src.indexOf("'主'") === -1);
  check('印そのものは残っている', /dslot__lock/.test(src));
  check('読み上げ用の説明がある', /aria-label/.test(src));
  check('★絵文字ではなくCSSで描いている',
    /\.dslot__lock::before/.test(css) && /\.dslot__lock::after/.test(css),
    '絵文字は端末ごとに姿が変わります');

  /* 追加できるカードの一覧は、デッキ側のマスより大きくします。
     デッキ側は8列で、1枠あたり約122pxです。 */
  const band = (css.match(/\.dband__card \{[\s\S]*?\}/) || [''])[0];
  const w = (band.match(/width:\s*(\d+)px/) || [])[1];
  check('★一覧のカードがデッキ側より大きい', Number(w) > 130, w + 'px（デッキ側は約122px）');
}
